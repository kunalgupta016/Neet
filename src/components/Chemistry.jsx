...
import QuestionsData from './Chemistry.json';
    ...
    const [questions, setquestions] = useState([]);
      ...
      const handleAnswer = (answer,index)=>{{
        if(answer==questions[currentIndex].correct_answer){
          setScore(score+10);
          setborderColor(prev => ({ ...prev, [index]: 'rgb(89, 228, 35)' }));
          
        }
        else{
          setborderColor(prev => ({ ...prev, [index]: 'rgb(255, 0, 0)' }));
        }
          setTimeout(() => {
            nextQuestion();
          }, 250);
          ...
          <div className='min-h-screen min-w-screen bg-[#F7F7FA]'>
            ...
            >
              <div className='container flex justify-between items-center'> {/* navbar ke liye*/ }
                ...
                <div> {/* Right Nav bar */}
                  <h1 className='lg:text-xl md:text-sm text-xs lg:font-bold '>Chemistry Quiz</h1>
                </div>
              </div>
            ...
            >
                ...
                {options.map((option,index)=>(
                  ...
                  style={{ borderColor: borderColor[index] || 'black' }}
